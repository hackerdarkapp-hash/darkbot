...
import { generateImage, editImage, analyzeImage, isEditInstruction } from "./image.js";
import {
...
function userKeyboard(): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("🎨 توليد صورة", "action:image")
    .text("💬 محادثة عامة", "mode:default").row()
    .text("🗑️ مسح محادثتي", "action:clear")
...
🖥️ البرمجة  •  🔍 البحث  •  ✍️ الكتابة
🎨 الصور  •  💬 المحادثة الذكية
...
export function createBot(): Bot<BotContext> {
  ...
  bot.command("image", async (ctx) => {
    ...
    if (!prompt) {
      ...
      await ctx.reply("🎨 اكتب وصف الصورة التي تريد توليدها:");
      return;
    }
    await handleImageGeneration(ctx, userId, prompt);
  });
  ...
  bot.command("code", async (ctx) => {
    ...
    ctx.session.awaitingPersona = false; ctx.session.awaitingImagePrompt = false;
  ...
  bot.callbackQuery("action:image", async (ctx) => {
    const userId = ctx.from?.id; if (!userId) return;
    ctx.session.awaitingImagePrompt = true;
    ctx.session.awaitingPersona = false;
    await ctx.answerCallbackQuery();
  ...
  bot.on("message:photo", async (ctx) => {
    ...
    if (!caption) {
      await ctx.reply(
        "🖼️ *أرسل الصورة مع تعليق:*\n\n" +
        "• للتحليل: اكتب سؤالك\n  مثال: `ما هذه المعادلة؟`\n\n" +
        "• للتعديل: ابدأ بكلمة تعديل\n  مثال: `عدّل الخلفية لغابة`",
        { parse_mode: "Markdown" }
      );
      return;
    }
    const isEdit = isEditInstruction(caption);
    const thinkingMsg = await ctx.reply(isEdit ? "🖌️ جاري تعديل الصورة..." : "🔍 جاري تحليل الصورة...");
    ...
    try {
      ...
      if (isEdit) {
        if (photoChatId) await ctx.api.sendChatAction(photoChatId, "upload_photo");
        const result = await editImage(imageBase64, "image/jpeg", caption);
        const editFile = new InputFile(Buffer.from(result.imageBase64, "base64"), `image.${result.mimeType.split("/")[1] ?? "png"}`);
        await ctx.replyWithPhoto(editFile, {
          caption: result.text?.slice(0, 1024) ?? `✅ ${caption}`,
        });
      } else {
  ...
  bot.on("message:text", async (ctx) => {
    ...
    if (ctx.session.awaitingImagePrompt) {
      ctx.session.awaitingImagePrompt = false;
      await handleImageGeneration(ctx, userId, text);
      return;
    }
...
async function handleImageGeneration(ctx: BotContext, userId: number, prompt: string): Promise<void> {
  const thinkingMsg = await ctx.reply("🎨 جاري توليد الصورة...");
  const chatId = ctx.chat?.id;
  try {
    if (chatId) await ctx.api.sendChatAction(chatId, "upload_photo");
    const result = await generateImage(prompt);
    if (chatId) await ctx.api.deleteMessage(chatId, thinkingMsg.message_id);
    const kb = isAdmin(userId) ? adminKeyboard() : userKeyboard();
    const file = new InputFile(Buffer.from(result.imageBase64, "base64"), `image.${result.mimeType.split("/")[1] ?? "png"}`);
    await ctx.replyWithPhoto(file, { caption: result.text?.slice(0, 1024) ?? `🎨 ${prompt}`, reply_markup: kb });
  } catch (err) {
    logger.error({ err, userId }, "Image generation error");
    if (chatId) await ctx.api.deleteMessage(chatId, thinkingMsg.message_id).catch(() => {});
    await ctx.reply(`❌ ${err instanceof Error ? err.message : "حدث خطأ أثناء توليد الصورة."}`);
  }
}
